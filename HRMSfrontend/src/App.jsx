...
// 👈 PASTE THIS DIRECTLY INSIDE renderContent() RIGHT ABOVE THE LAST 'return null;' STATEMENT:
    if (activeTab === 'ID Cards') {
      return (
        <div className="xl:col-span-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Right Panel: Interactive ID Badge & QR Scanner Data */}
          <div className="space-y-6">
              ...
              {selectedQrEmployee ? (
                <>
                  ...
                  <div className="w-full space-y-4">
                    ...
                    {/* Dynamic Verification QR Generation */}
                    <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl inline-block shadow-inner mx-auto border border-slate-100 dark:border-slate-700">
                      <img 
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(
                          `Verified HRise Profile:\n\nID: ${selectedQrEmployee.id}\nName: ${selectedQrEmployee.name}\nRole: ${selectedQrEmployee.role}\nDepartment: ${selectedQrEmployee.dept}\nManager: ${selectedQrEmployee.manager}\nJoining Date: ${selectedQrEmployee.joiningDate}`
                        )}`} 
                        alt="Employee verification link matrix" 
                        className="w-40 h-40 object-contain block dark:invert-[0.05] mix-blend-multiply dark:mix-blend-normal"
                      />
                    </div>

                    <p className="text-[10px] text-slate-400 max-w-xs mx-auto">
                      Scan code using any generic camera tool to extract real-time enterprise deployment details.
                    </p>
                ...
                <div className="text-center text-slate-400 space-y-2">
                  ...
                  <p className="text-[11px] text-slate-400 max-w-[200px] mx-auto">Click any record on the left grid directory to view live credentials.</p>
                </div>
              )}
            </div>
          </div>
