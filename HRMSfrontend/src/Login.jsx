...
const GoogleIcon = () => (
  <svg viewBox="0 0 48 48" aria-hidden="true" className="h-5 w-5">
    ...
    <path
      fill="#FF3D00"
      d="m6.3 14.7 6.6 4.8C14.7 15.1 19 12 24 12c3.1 0 5.8 1.2 8 3l5.7-5.7C34 6.1 29.3 4 24 4c-7.7 0-14.3 4.3-17.7 10.7Z"
    />
    <path
      fill="#4CAF50"
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-[10px] leading-3 text-white/78">
            {item.detail}
          </p>
        </div>
      </div>
      ...
      <div className="relative z-10 flex h-screen flex-col justify-between p-10 xl:p-12">
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-24 lg:px-32 pointer-events-none">
          ...
          >
            ...
            >
              Empower your teams and streamline human resources with next-gen
              management controls.
            </motion.p>
          </motion.div>
        </div>
...
function PasswordStrength({ password }) {
  ...
  const color =
    passed <= 1 ? "bg-rose-500" : passed <= 3 ? "bg-amber-500" : "bg-teal-500";
...
export default function Login({ onLogin }) {
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        notify(
          `Login successful! Welcome back, ${user.name || "there"}.`,
          "success",
        );
  ...
  const handleMicrosoftLogin = () => {
    setIsLoading(true);
    try {
      const mockMicrosoftPopup = window.open(
        "",
        "MicrosoftLogin",
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Direct call to your Express backend endpoints
      const endpoint =
      ...
      if (!res.ok) {
        notify(data.message || "Authentication failed.", "error");
        setIsLoading(false);
        return;
      }
      ...
      if (data.token) {
        localStorage.setItem("hrise_auth_token", data.token);
      }

      // Complete login in UI and pass authenticated user data to App.jsx
      window.setTimeout(() => {
      ...
      notify(
        "Cannot connect to backend server. Make sure port 5000 is active.",
        "error",
      );
      setIsLoading(false);
    }
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {mode === "register" &&
                  "Enter your information to set up your profile."}
                {mode === "forgot" &&
                  "Enter your verified work email to recover your enterprise credentials."}
                {mode === "otp" &&
                  "Enter your work email to receive a secure sign-in verification code."}
                {mode === "login" &&
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <motion.button
                      ...
                      className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-slate-600 no-underline cursor-pointer"
                    >
            ...
            <motion.div
              whileHover={{ scale: 1.01 }}
              className="text-center pt-2 space-y-2 cursor-pointer"
            >
              {mode === "login" && (
                <>
          ...
          <motion.p
            ...
            animate={{ opacity: 1 }}
            transition={{ delay: 1 }}
            className="absolute bottom-4 text-[11px] text-slate-400 text-center w-full max-w-md no-underline cursor-pointer"
          >
            By signing in, you agree to our{" "}
            <a
              href="#"
              className="no-underline hover:text-indigo-600 cursor-pointer"
            >
              Terms of Service
            </a>{" "}
            and{" "}
